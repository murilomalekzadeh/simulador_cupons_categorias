def calculo_final_cupons(df,
    ...
    def calculo_tabela(df, agrupamento):
        ...
        if agrupamento == 'Total':
            tabela_1 = (df.agg({'MISSAO_COMPRA': 'count', 'taxa_conversao': 'sum', 'margem_natural': 'sum', 'margem_com_cupom': 'sum', 'reducao_margem': 'sum' }).to_frame(name='Total') ) 
            tabela_1.index = ['Total de Disparos', 'Uso de cupons', 'Margem Natural', 'Margem com Cupom', 'Redução de Margem']
        ...
        if agrupamento == 'Total':
            tabela_1.loc['Taxa de Uso do Cupom'] = tabela_1.loc['Uso de cupons'] / tabela_1.loc['Total de Disparos']
            tabela_1.loc['Margem Natural Media'] = tabela_1.loc['Margem Natural'] / tabela_1.loc['Uso de cupons']
            tabela_1.loc['Margem com Cupom Media'] = tabela_1.loc['Margem com Cupom'] / tabela_1.loc['Uso de cupons']
            tabela_1.loc['Diferença de Margem Media'] = tabela_1.loc['Redução de Margem'] / tabela_1.loc['Uso de cupons']
        else:
            tabela_1['Taxa de Uso do Cupom'] = tabela_1['Uso de cupons'] / tabela_1['Total de Disparos']
            tabela_1['Margem Natural Media'] = tabela_1['Margem Natural'] / tabela_1['Uso de cupons']
            tabela_1['Margem com Cupom Media'] = tabela_1['Margem com Cupom'] / tabela_1['Uso de cupons']
            tabela_1['Diferença de Margem Media'] = tabela_1['Redução de Margem'] / tabela_1['Uso de cupons']

        tabela_exibicao = tabela_1.copy()
        if agrupamento != 'Total':
            tabela_exibicao = tabela_exibicao.set_index(agrupamento).T
        ...
        # Garante que cada célula pode ter qualquer tipo (int, float, str) sem conflito de dtype por coluna
        tabela_exibicao = tabela_exibicao.astype(object)
