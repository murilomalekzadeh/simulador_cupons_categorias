...
df = df.rename(columns={
    ...
    'MACROSSEGMENTO_PEOPLESCOPE' : 'Macrossegmento PeopleScope'})
...
if st.button("Executar Simulação"):
    ...
    try:
        ...
        with aba1:
            st.dataframe(tb_total, use_container_width=True, hide_index=True)

        with aba2:
            st.dataframe(tb_segmento_ps, use_container_width=True, hide_index=True)
            
        with aba3:
            st.dataframe(tb_segmento_estrategico, use_container_width=True, hide_index=True)
